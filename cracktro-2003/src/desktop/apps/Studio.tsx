...
const PALETTE = ["#ffffff","#d7ff00","#00ffd7","#00a2ff","#b667ff","#ff3ca6","#ff9f1a","#bbbbbb","#000000"];
    ...
    <div className="win">
      ...
      <div className="toolbar">
        <div className="swatches">
          {PALETTE.map(p => (
            <button key={p} className={`sw ${p===color?"sel":""}`} style={{background:p}} onClick={()=>setColor(p)} />
          ))}
        </div>
        <div className="tools">
      ...
      <style jsx>{`
        .win{ width: 560px; background:#0f0f0f; color:#e6e6e6; border:1px solid #444;
        ...
        .titleClose:focus{ outline:none; } .titleClose:active{ box-shadow:inset 1px 1px 0 #000, inset -1px -1px 0 #6a6a6a; }
        .toolbar{ display:flex; justify-content:space-between; align-items:center; gap:10px; padding:6px; border-bottom:1px solid #333;
                  background:#111; box-shadow:inset 1px 1px 0 #000, inset -1px -1px 0 #6a6a6a; }
        .swatches{ display:flex; gap:6px; }
        .sw{ width:18px; height:18px; border:1px solid #000; box-shadow:inset 1px 1px 0 #6a6a6a, inset -1px -1px 0 #000, 0 0 0 1px #000; cursor:pointer; }
        ...
        canvas{ image-rendering: pixelated; border:1px solid #333; box-shadow:inset 1px 1px 0 #000, inset -1px -1px 0 #6a6a6a; }
      `}</style>
    </div>
  );
}
